...
public class PngExporter implements Exporter<String> {
	...
	public boolean exportToFile(GameMap map, File file) {
		try {
			...
			out = new BufferedImage(map.getWidth() * map.getTileSize(),
									map.getHeight() * map.getTileSize(),
											BufferedImage.TYPE_INT_ARGB);
			export(map);
			
			ImageIO.write(out.getSubimage(bounds[0] * map.getTileSize(),
											bounds[1] * map.getTileSize(),
											(1 + bounds[2] - bounds[0]) * map.getTileSize(),
											(1 + bounds[3] - bounds[1]) * map.getTileSize()),
											"PNG", file);
	...
	@Override
	public String export(TileLayer tileLayer, ExporterData data) {
		return "";
	}
	...
	@Override
	public String export(Area area, ExporterData data) {
		return "";
	}

	@Override
	public String export(GameObject gameObject, ExporterData data, float depth) {
		return "";
	}
